...
Select distinct t3.empid, t3.name
From DigitalDisplay t1 join Specializes t2 on (t1.modelNo = t2.modelNo) 
Join TechnicalSupport t3 on (t2.empid = t3.empid)
Where t1.schedulerSystem = 'Random';
...
--11
select t2.empId, t2.name from Salesman t2 where t2.empId in (
select v2.empId from (
select v1.empId, rank() over (order by num_sales) rn from (
select t1.empId,count(*) num_sales from Purchases t1 group by t1.empId) v1) v2 where v2.rn > 1
);
